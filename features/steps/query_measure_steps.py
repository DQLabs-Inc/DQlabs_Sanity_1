...
def login_using_json_for_asset_delete(context):
    ...
    context.query_measure.login(username, password)  # Using credentials from JSON
    time.sleep(10)

@then('navigate to assets main page')
def click_asset_page(context):
    context.query_measure.asset_page()
    time.sleep(10)

@then('select list view page')
def click_list_view(context):
    context.query_measure.list_view()
    time.sleep(5)

@then('click on icon to search')
def click_search_icon(context):
    context.query_measure.search_icon()
    time.sleep(5)

@then('enter "{text}" in the search box')
def enter_search_asset(context, text):
    context.query_measure.search_asset(text)
    time.sleep(5)

@then('select asset to add measures')
def select_delete_asset(context):
    context.query_measure.select_asset()
    time.sleep(5)
...
#
#     # ✅ Click the dropdown field first
...
#
#     # ✅ Now, wait for the dropdown list to appear
#     try:
...
#         return
#
#     # ✅ Wait for all options inside the dropdown
#     try:
...
#
#     time.sleep(10)  # Allow selection to reflect


# @Then('select attribute value "{text}"')
# def step_select_conditional_attribute(context, text):
#     # context.save_measures_page.CONDITIONAL_ATTRIBUTE_DROP_DOWN()
#     context.save_measures_page.select_conditional_attribute(text)
#     time.sleep(5)
#
# @Then('select condition value "{text}"')
# def select_condition_value(context, text):
#     # context.save_measures_page.CONDITIONAL_ATTRIBUTE_DROP_DOWN()
#     context.save_measures_page.select_condition_value(text)
#     time.sleep(5)


@then('select query measure')
def select_query_measure(context):
    context.query_measure.select_query_measure()
    time.sleep(5)

# @then('enter_measure_name')
# def enter_measure_name(context, text):
#     context.save_measures_page.enter_measure_name()
#     time.sleep(5)

@then('enter_measure_name"{text}"')
def enter_measure_name(context, text):
    context.query_measure.enter_measure_name(text)
    time.sleep(5)

@then('provide_query "{text}"')
def provide_query(context, text):
    context.query_measure.provide_query(text)
    time.sleep(5)
